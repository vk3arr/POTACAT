...
class Ft8Engine extends EventEmitter {
  ...
  _onWorkerMessage(msg) {
    ...
    if (msg.type === 'decode-result') {
      this._cycleNumber++;
      console.log(`[JTCAT] Decode result: ${(msg.results || []).length} decodes`);
      this.emit('decode', {
  ...
  _onCycleBoundary() {
    ...
    const maxSample = Math.max(...Array.from(samples.slice(0, 1000)).map(Math.abs));
    console.log(`[JTCAT] Cycle boundary: sending ${samples.length} samples to worker, max=${maxSample.toFixed(4)}, offset=${off}`);
