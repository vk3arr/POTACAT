...
function registerCloudIpc(ctx) {
  ...
  ipcMain.handle('cloud-get-status', async () => {
    try {
      ...
      const [syncStatus, subStatus] = await Promise.all([
        sync.getStatus().catch((e) => { console.error('Cloud status fetch error:', e.message); return null; }),
        sync.getSubscriptionStatus().catch((e) => { console.error('Cloud sub fetch error:', e.message); return null; }),
      ]);
  ...
  ipcMain.handle('cloud-sync-now', async () => {
    try {
      ...
      const settings = ctx.getSettings();
      settings.cloudLastSyncTimestamp = sync.lastSyncTimestamp;
      ctx.saveSettings(settings);
