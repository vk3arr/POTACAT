  ...
  _parseMeterPacket(buf) {
    ...
    for (let i = 0; i < numMeters; i++) {
      ...
      if (meterId === this._smeterMeterId) {
        ...
        if (swr > 1.0) {
          // Convert SWR ratio to 0-255 scale matching RM1 format
    ...
    if (line.includes('.src=') && line.includes('.nam=')) {
      ...
      for (const [id, m] of Object.entries(meters)) {
        if (m.src && m.nam) {
          ...
          // Find SWR: src=TX-, nam=SWR
          if (m.src === 'TX-' && m.nam === 'SWR' && !this._swrMeterId) {
            this._swrMeterId = numId;
            console.log(`[SmartSDR] SWR meter: meter id ${numId}`);
          }
        }
      }
    }
