...
class SmartSdrClient extends EventEmitter {
  ...
  _doConnect() {
    ...
    sock.on('connect', () => {
      ...
      // Subscribe to slice meters — S-meter (id 14) and SWR come via meter updates
  ...
  _parseStatusMessage(line) {
    // Log any meter status updates to discover the value format
    if (line.includes('meter')) {
      // Don't log the initial meter definitions (they have .src= and .nam=)
      if (!line.includes('.src=')) {
        console.log(`[SmartSDR] meter update: ${line.slice(0, 200)}`);
      }
    }
