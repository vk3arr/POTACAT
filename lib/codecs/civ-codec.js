...
class CivCodec extends EventEmitter {
  ...
  constructor(model, writeFn) {
    ...
    // Track the mode we commanded (not yet confirmed by polling)
    this._pendingModeByte = null;
    this._pendingDataMode = false;
  }
  ...
  setMode(modeName, freqHz) {
    ...
    // Remember what we commanded so setFilterWidth() can use it
    this._pendingModeByte = resolved.civMode;
    this._pendingDataMode = !!resolved.dataMode;

    // Set mode (cmd 0x06)
    this._sendCmd(0x06, null, [resolved.civMode, 0x01]); // filter=1 (default)
  ...
  setFilterWidth(hz) {
    // CI-V cmd 0x06 payload is [modeByte, filterByte] — it sets mode AND filter.
    // Use the mode we just commanded (pending), falling back to polled mode.
    // Without this, a stale/null _lastModeByte can override the mode we just set.
    const modeByte = this._pendingModeByte || this._lastModeByte;
    if (modeByte == null) return; // no mode known yet — skip rather than guess wrong

    if (hz <= 500) {
      this._sendCmd(0x06, null, [modeByte, 0x03]); // narrow
    } else if (hz <= 1800) {
      this._sendCmd(0x06, null, [modeByte, 0x02]); // medium
    } else {
      this._sendCmd(0x06, null, [modeByte, 0x01]); // wide
    }

    // cmd 0x06 resets data mode — re-send if we just set it
    if (this._pendingDataMode) {
      this._sendCmd(0x1A, 0x06, [0x01, 0x01]); // data mode ON, filter 1
    }
  }
